...
export const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, isOpen = false, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`w-64 border-r border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl flex flex-col justify-between p-4 fixed md:sticky top-0 h-screen z-50 shrink-0 transition-transform duration-300 md:translate-x-0 ${
        ...
        <div>
          <div className="flex items-center justify-between px-2 py-3 mb-4">
            ...
            {onClose && (
              <button
                onClick={onClose}
                className="md:hidden p-1.5 rounded-xl text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-900 transition"
              >
                <X className="w-5 h-5" />
              </button>
            )}
